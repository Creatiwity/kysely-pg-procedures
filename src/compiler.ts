...
import type { RlsEnableDef, PolicyDef } from './rls.js'
import { compileRlsEnable, compilePolicyBlock } from './rls.js'
...
export function compileAll(
  defs: Array<ProcedureDefinition | TriggerDefinition | RlsEnableDef | PolicyDef>,
  opts?: CompileOpts,
): string {
  const lines = ['-- Generated by @mesalia/kysely-pg-procedures', '-- DO NOT EDIT MANUALLY', '']
  const compiled = new Set<string>()

  // Order: RLS enables first, then policies, then procedures, then triggers
  const rlsEnables = defs.filter((d): d is RlsEnableDef => d._tag === 'RlsEnable')
  const policies = defs.filter((d): d is PolicyDef => d._tag === 'Policy')
  const procedures = defs.filter((d): d is ProcedureDefinition => d._tag === 'Procedure')
  const triggers = defs.filter((d): d is TriggerDefinition => d._tag === 'Trigger')

  for (const def of rlsEnables) {
    const key = `rls:${def.table}`
    if (!compiled.has(key)) {
      lines.push(compileRlsEnable(def))
      lines.push('')
      compiled.add(key)
    }
  }
  ...
  for (const def of policies) {
    const key = `policy:${def.opts.name}`
    if (!compiled.has(key)) {
      lines.push(compilePolicyBlock(def))
      lines.push('')
      compiled.add(key)
    }
  }
